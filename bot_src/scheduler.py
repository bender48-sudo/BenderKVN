        ...
        if notify_mark is not None:
            try:
                ...
                if access_kind == "wallet" and BOT_PAYMENTS_LIVE:
                    if notify_mark > 0:
                        txt = (
                            f"Доступ заканчивается через {days_left} дней.\n\n"
                            f"Баланс: {bal:.0f} ₽. Пополни, чтобы не прерываться."
                        )
                    else:
                        txt = (
                    ...
                    if notify_mark > 0:
                        ...
                        txt = (
                            ...
                            "Конфигурация сохранена — ничего перенастраивать не нужно."
                        )
                else:
                    if notify_mark > 0:
                        txt = (
                            f"Доступ заканчивается через {days_left} дней.\n\n"
                            "Если нужна помощь — напиши нам."
                        )
                    else:
                        txt = "Доступ завершился. Напиши нам — разберёмся."
                await bot.send_message(
