...
def subscription_unavailable(telegram_id: int) -> dict:
    ...
    if user and not user.get("agreed_to_terms"):
        return {
            ...
            "message": (
                "Сначала в боте нажмите «Принимаю», затем «Получить настройку» или "
                "активируйте бесплатный период в главном меню."
            ),
            "bot_url": bot_url,
        }
    ...
    if keys and trial_used:
        return {
            ...
            "message": (
                "Срок доступа истёк. В боте нажмите «Пополнить баланс». "
    ...
    if keys:
        return {
            ...
            "message": "Не удалось получить ссылку. Откройте бота → «Получить настройку» или поддержку.",
            "bot_url": bot_url,
        }
    if not trial_used:
        return {
            "error": "no_subscription",
            "message": (
                "Сначала активируйте доступ в боте: «Получить настройку» или "
                "бесплатный период в главном меню."
            ),
            "bot_url": bot_url,
        }
    return {
        "error": "no_subscription",
        "message": "В боте нажмите «Пополнить баланс» в главном меню.",
        "bot_url": bot_url,
    }
