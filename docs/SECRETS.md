...
| *(локально, не в git)* | `.secrets/yookassa.env` или корень репо **`secret_key 2`** | Для dev/теста бота на машине владельца; подхватывает **`shop_bot.local_env.load_local_env()`** при старте. Шаблон: **`.secrets/yookassa.env.example`**. | ЛК YooKassa. | **Не пушить**; на AMS — только `/opt/remna-shop/.env`. |
