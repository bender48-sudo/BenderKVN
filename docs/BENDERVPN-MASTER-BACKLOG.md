...
| ID | Sev | Area | Title | Problem | Decision | Phase | Impact | Evidence | Files/modules | Acceptance | Checks | Deploy? | Owner? | Blocked by | Status |
|----|-----|------|-------|---------|----------|-------|--------|----------|---------------|------------|--------|---------|--------|------------|--------|
| VPN-REL-001 | P0 | VPN reliability | Unstable reconnect loop diagnostic | Users report disconnect/reconnect | N/A | **Now** | Core product value | User reports; `VPN-INCIDENT-LESSONS` | ops probes, panel, Happ | Read-only root-cause ranking; no routing PATCH in audit | AUDIT-001 gate | No | **Yes** for patch | — | OPEN |
| VPN-ARCH-001 | P1 | VPN architecture | Full VPN architecture audit | Happ/Auto/sub/DNS/routing/failover map incomplete for product track | N/A | **Now** read-only | Informed fixes | `BACKLOG-VPN-FULL-AUDIT`; stability resolution | ops/, panel | Architecture map doc | AUDIT-002 | No | **Yes** for patch | VPN-REL-001 | OPEN |
