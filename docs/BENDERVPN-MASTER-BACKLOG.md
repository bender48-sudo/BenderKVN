...
| — | **NEW-INDEPENDENT-EXIT-PATH-001** | P0 | **DONE — PATH A (owner-gated smoke)** | nl-node-1 VALIDATED as the 2nd INDEPENDENT exit candidate: remnanode VLESS/REALITY exit, **no** hysteria forwarder, own egress (`ep_2a1adf7b`) DISTINCT from LV (`ep_82c23da5`) & RU shared upstream (`ep_4b84b15b`). Formalized **INDEPENDENT_EXIT_PATH_V1** (shared-upstream exits collapse to 1; ≥2 needed for PUBLIC_PROD); registry `independent_exit_candidate`/`acceptance_status`; model `is_independent_exit*`; validator `independent_exit_paths`; canary artifact `.local/independent_exit_nl_canary.*`. NOT counted (staging, eligible=false) → `independent_exit_paths=1`. Owner phrase: **APPROVE NL INDEPENDENT EXIT TRAFFIC SMOKE** → [`NEW-INDEPENDENT-EXIT-PATH-2026-06-17.md`](NEW-INDEPENDENT-EXIT-PATH-2026-06-17.md) |
