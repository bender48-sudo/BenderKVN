...
| **NL as 2nd independent exit (2026-06-17)** | **Candidate validated, not yet counted** | NEW-INDEPENDENT-EXIT-PATH-001: nl-node-1 runs remnanode VLESS/REALITY exit, **no** hysteria forwarder, egress `ep_2a1adf7b` DISTINCT from LV `ep_82c23da5` & RU shared upstream `ep_4b84b15b` → real provider/geo/egress independence. `independent_exit_candidate=true`, but staging + `delivery_path_eligible=false` → `independent_exit_paths` stays **1**. Only blocker: owner A2/A4 traffic smoke. This is the real path to `delivery_path_nodes=2`. |
