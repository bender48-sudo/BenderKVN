...
| **P1-RED-LOG-01** | **Логи edge подписки / Caddy**: глобально **`log_skip`** (или эквивалент) для URI с токеном подписки; политика **retention** access‑логов; запрет полного URL подписки в центральных логах. | **`ops/fix-caddy-security.sh`** / патчи **`Caddyfile`** на всех edge; smoke grep по логам = нет сыро́го subscription path. |
...
| **P2-CHORE-SUB-ENV** | **`monitor.sh`** — smoke GET подписки всё ещё с захардкоженным URL. | Тот же шаблон, что **`daily-report.sh`**: `SUB_PUBLIC_ORIGIN` / **`SUB_MONITOR_PROBE_URL`** или source из `/etc/bvpn/balancer.env` (не секрет). |
...
| **P2-ENG-DRIFT-CHECK-01** | **`ops/drift-check.py`**: нестабильные **TIMEOUT** при md5 **`/etc/bvpn/balancer.env`** и **`ru-monitor.env`** на LV (защищённые файлы / SSH). | Retry/backoff **или** один сеансовый вызов с чтением файлов; документировать в **`docs/DEPLOY.md`**. |
