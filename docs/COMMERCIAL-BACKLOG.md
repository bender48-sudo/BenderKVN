...
| 2026-06-04 | **Фаза 13 NL (прод):** LV **`VPN_VERIFY_GATE_OK`** + **`HAPP_GEOSITE_GUARD_OK`**; **`NL_NODE_HEALTH_OK`** (panel+RU probe на LV, token из **`ru-monitor.env`**); NL BBR **`99-tcp-bbr.conf`** → **bbr+fq**; legacy relay→NL :9443 **disabled×3**. Репо: probe, roadmap, runbook relay-NL-443 PoC. **NEXT:** **VPN-AUD-275**. |
