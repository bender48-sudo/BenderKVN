...
**gen=25:** Intl_Direct multipath (IG/TG); Super_Balancer оставался relay-only SPOF.

**gen=26 (28.05):** оба балансировщика → `RU_MULTIPATH_SELECTOR` (11 путей):

`proxy … proxy-4` (LV) + `proxy-5 … proxy-7` (RELAY) + `proxy-8 … proxy-11` (NL)

Исключены: `["proxy"]` all-14 (Q132 footgun), relay-only 5..7 (SPOF), RELAY-NL 12..14.

**Verify:** `python ops/verify_vpn_balancer_profile.py` → `VPN_BALANCER_PROFILE_OK`
