...
|------|------|--------|----------|---------------|---------------|---------------|-----------------|----------------|
| **G1** | VPN stability / Happ / sleep-resume / browser SaaS / **CLIENT-STABILITY** | **PARTIAL** | Track A sleep OPEN; **Track D Happ DirectIp fix in repo** (geoip:ru removed) — **owner TUN retest + optional happRouting deploy pending**; Track B Proxy OPEN | Desktop + SaaS work = **yes** | **Yes** if desktop sold | **Yes** | Owner retest §INCIDENT-DIAG-2026-06-12; `happ_routing_directip_guard.py` | Waive desktop/SaaS support? |
...
9. G12 capacity dashboard + **node capacity acceptance** (≥2 **verified** delivery-path nodes after NL controlled smoke if pursuing A2/A4) before approaching 300 active configs/devices — **ACQ-SLOTS-001** / **ACQ-TRIAL-CAP-001**; **VPN-ARCH-001** (QUALITY-PROOF-001: NL pre-qualified, **not** active capacity); **VPN-NODE-RUNBOOK-001**
...
| **NL active capacity** | **QUALITY-PROOF-001 DONE** — NL pre-qualified (RU relay→NL PASS, infra OK); **not** active until A2/A4 controlled smoke + post-inclusion audit; next step after **MONITOR-FLAP** soak, not blind PATCH or decom |
