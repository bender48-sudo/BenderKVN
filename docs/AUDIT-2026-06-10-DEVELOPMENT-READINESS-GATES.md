...
**INCIDENT-003 (2026-06-10):** Laptop **sleep/resume** — another VPN survives; BenderVPN/Happ breaks networking until reboot sometimes. **INCIDENT-DIAG-003-004** (2026-06-10 Happ report): TUN crash loop before reboot — stronger evidence, **AFTER-BROKEN** snapshots still missing. **CLIENT-STABILITY-001** (2026-06-10): Happ UI confirms TUN daemon failure; **Track B** — Bender Proxy fails while other VPN Proxy works (separate investigation). **INCIDENT-DIAG-2026-06-12:** owner report(2) — TUN OK but relay storm; root cause `geoip:ru` in Happ DirectIp. **Fix repo** `0258d00` + **owner local retest PASS 2026-06-12** (fast TUN connect, intl services OK). **Prod `patch_happ_routing.py --apply` pending** explicit approval. Track A sleep/resume + Track B Proxy **still OPEN**. See [`INCIDENT-DIAG-2026-06-12-HAPP-TUN-ACTIVE-FAILURE.md`](INCIDENT-DIAG-2026-06-12-HAPP-TUN-ACTIVE-FAILURE.md) §11.1–11.2.
