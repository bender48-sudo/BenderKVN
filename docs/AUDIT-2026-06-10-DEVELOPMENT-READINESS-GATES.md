...
**QA-PAYMENT-DRYRUN-001 (2026-06-10):** `bot_src/yookassa_dryrun.py` — dummy `payment_create` responses (`qa-pay-*` id, `https://sandbox.invalid/pay/…`) when `BVPN_QA_DRY_RUN_PAYMENTS=1`; no balance mutation — **implemented repo, not deployed**. Webhook simulator remains OPEN.
...
**INCIDENT-003 (2026-06-10):** Laptop **sleep/resume** — another VPN survives; BenderVPN/Happ breaks networking until reboot sometimes. **INCIDENT-DIAG-003-004** (2026-06-10 Happ report): TUN crash loop before reboot — stronger evidence, **AFTER-BROKEN** snapshots still missing. **CLIENT-STABILITY-001** (2026-06-10): Happ UI confirms TUN daemon failure; **Track B** — Bender Proxy fails while other VPN Proxy works (separate investigation). **INCIDENT-DIAG-2026-06-12:** owner report(2) — TUN OK but relay storm; **root cause:** `geoip:ru` in Happ routing **DirectIp** (not subscription template). **Fix repo:** `ops/happ_routing_directip_guard.py` + remove geoip:ru from profile generator. **Owner retest** with routing ON required; **`patch_happ_routing.py --apply`** only after approval. [`INCIDENT-2026-06-10-VPN-LAPTOP-SLEEP-RESUME.md`](INCIDENT-2026-06-10-VPN-LAPTOP-SLEEP-RESUME.md), [`INCIDENT-DIAG-2026-06-10-WINDOWS-SLEEP-RESUME-MAIL.md`](INCIDENT-DIAG-2026-06-10-WINDOWS-SLEEP-RESUME-MAIL.md), [`INCIDENT-DIAG-2026-06-10-HAPP-TUN-DAEMON-PROXY-FALLBACK.md`](INCIDENT-DIAG-2026-06-10-HAPP-TUN-DAEMON-PROXY-FALLBACK.md), [`INCIDENT-DIAG-2026-06-12-HAPP-TUN-ACTIVE-FAILURE.md`](INCIDENT-DIAG-2026-06-12-HAPP-TUN-ACTIVE-FAILURE.md). **No prod mutation** until routing profile audit + owner routing-OFF confirmation + CLIENT-SMOKE-001..003.
