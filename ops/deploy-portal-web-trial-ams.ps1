...
$Files = @(
    ...
    @{ Local = "bot_src\web_referral.py"; Remote = "/tmp/web_referral.py" },
    @{ Local = "bot_src\webhook_server\app.py"; Remote = "/tmp/webhook_app.py" }
)
foreach ($f in $Files) {
...
r = Path(r'$RepoRoot')
for n in ('portal_web_trial.py', 'web_referral.py'):
    ast.parse((r / 'bot_src' / n).read_text(encoding='utf-8'))
ast.parse((r / 'bot_src/webhook_server/app.py').read_text(encoding='utf-8'))
"@
...
sed -i 's/\r$//' /tmp/portal_web_trial.py /tmp/web_trial_db.py /tmp/web_tg_bind.py /tmp/web_referral.py /tmp/webhook_app.py
test -f "$SB/portal_web_trial.py" && cp "$SB/portal_web_trial.py" "$SB/portal_web_trial.py.before-web-trial-$ts"
test -f "$SB/web_trial_db.py" && cp "$SB/web_trial_db.py" "$SB/web_trial_db.py.before-web-trial-$ts"
test -f "$SB/web_tg_bind.py" && cp "$SB/web_tg_bind.py" "$SB/web_tg_bind.py.before-web-trial-$ts"
test -f "$SB/web_referral.py" && cp "$SB/web_referral.py" "$SB/web_referral.py.before-web-trial-$ts"
...
install -m 0644 /tmp/web_referral.py "$SB/web_referral.py"
...
docker cp /tmp/web_referral.py remna-shop-bot:/app/src/shop_bot/web_referral.py
...
docker exec remna-shop-bot grep -q ref_code /app/src/shop_bot/webhook_server/app.py
docker exec remna-shop-bot test -f /app/src/shop_bot/portal_web_trial.py
docker exec remna-shop-bot test -f /app/src/shop_bot/web_referral.py
